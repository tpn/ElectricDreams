...
ASPPlayerCameraManager::ASPPlayerCameraManager(const FObjectInitializer& ObjectInitializer)
...
void ASPPlayerCameraManager::DisplayDebug(class UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
	...
	else
	{
		...
		{
			FActiveSPCamera& StackEntry = CameraBlendStack[CamModeIndex];
...
USPCameraMode* ASPPlayerCameraManager::GetCurrentCameraMode()
{
	if (ViewTarget.Target)
	{
		...
		USPCameraMode* const BestCamera = CameraModeInstances[InstanceIdx].CameraMode;
		return BestCamera;
	}
...
void ASPPlayerCameraManager::UpdateCameraInStack(int32 StackIdx, float DeltaTime, FTViewTarget& OutVT)
{
	if (CameraBlendStack.IsValidIndex(StackIdx))
	{
		FActiveSPCamera& CamEntry = CameraBlendStack[StackIdx];
		...
		else
		{
			...
			{
				FSPCameraModeInstance& ModeInstance = CameraModeInstances[InstanceIdx];
				if ((StackIdx == 0) && bSkipNextInterpolation)
				{
					ModeInstance.CameraMode->SkipNextInterpolation();
				}
...
APlayerController* ASPPlayerCameraManager::GetOwningPC() const
{
	return Cast<APlayerController>(PCOwner);
}

void ASPPlayerCameraManager::TransitionToAltCamera()
{
	if (APlayerController* const PC = GetOwningPC())
	{
...
void ASPPlayerCameraManager::TransitionFromAltCamera()
{
	if (APlayerController* const PC = GetOwningPC())
	{
...
void ASPPlayerCameraManager::ConfigureAlternateCamera(TSubclassOf<USPCameraMode> NewAltCameraMode, AActor* NewAltViewTarget, float NewAltCameraTransitionTime)
{
...
void ASPPlayerCameraManager::UpdateViewTarget(struct FTViewTarget& OutVT, float DeltaTime)
{
	...
	else
	{
		...
		USPCameraMode* DeactivatedCamera = nullptr;
		...
		if ((CameraBlendStack.Num() == 0) || (CameraBlendStack[0].Camera != BestCamera))
		{
			...
			for (int32 CamModeIndex = 0; CamModeIndex < CameraBlendStack.Num(); CamModeIndex++)
			{
				FActiveSPCamera& CamEntry = CameraBlendStack[CamModeIndex];
				if (CamEntry.Camera == BestCamera)
				{
					...
					{
						// restart transition the other way
						float TransitionTime = GetModeTransitionTime(BestCamera);
						if (TransitionTime > 0.f)
						{
							CamEntry.TransitionUpdateRate = (1.f / TransitionTime);
						}
					}
					break;
				}
			...
			BestCamera->OnBecomeActive(OutVT.Target, DeactivatedCamera, bAlreadyInStack);
		}

		if (bSkipBlendsOnNextUpdate)
		{
			// set active cam to full weight
			if (CameraBlendStack.Num() > 0)
			{
				CameraBlendStack[0].BlendWeight = 1.f;
				CameraBlendStack[0].TransitionAlpha = 1.f;
				CameraBlendStack[0].TransitionUpdateRate = 0.f;
			}

			// set all other cameras to no weight. they will get cleaned up eventually
			for (int32 CamModeIndex = 1; CamModeIndex < CameraBlendStack.Num(); CamModeIndex++)
			{
				FActiveSPCamera& CamEntry = CameraBlendStack[CamModeIndex];
				CamEntry.BlendWeight = 0.f;
				CamEntry.TransitionAlpha = 0.f;
				CamEntry.TransitionUpdateRate = 0.f;
			}

			bSkipBlendsOnNextUpdate = false;
		}
		...
		{
			FActiveSPCamera& LastCamEntry = CameraBlendStack.Last();
			...
			{
				FActiveSPCamera& CamEntry = CameraBlendStack[Idx];
...
void ASPPlayerCameraManager::CleanUpOutdatedCameraModeInstances()
{
...
int32 ASPPlayerCameraManager::FindOrCreateCameraModeInstance(TSubclassOf<USPCameraMode> CameraModeClass, AActor* InViewTarget)
{
	for (int Idx = 0; Idx < CameraModeInstances.Num(); ++Idx)
	{
		const FSPCameraModeInstance& Inst = CameraModeInstances[Idx];
	...
	USPCameraMode* const NewCameraMode = NewObject<USPCameraMode>(this, CameraModeClass);

	NewCameraMode->PlayerCamera = this;

	FSPCameraModeInstance NewInstance;
...
int32 ASPPlayerCameraManager::GetBestCameraMode(AActor* Target)
{
	// Find which SPCameraMode Class we should be using for this ViewTarget.
	TSubclassOf<USPCameraMode> CameraModeClass = DetermineBestCameraClass(Target);
	if (!CameraModeClass)
	{
		// Making sure we always have a valid class
		CameraModeClass = USPCameraMode::StaticClass();
	}
...
TSubclassOf<USPCameraMode> ASPPlayerCameraManager::GetCameraClassForCharacter_Implementation(const AActor* InViewTarget) const
{
	return nullptr;
}

float ASPPlayerCameraManager::GetModeTransitionTime(USPCameraMode* ToMode) const
{
	if (ToMode)
	{
...
TSubclassOf<USPCameraMode> ASPPlayerCameraManager::DetermineBestCameraClass(AActor const* InViewTarget) const
{
	...
	TSubclassOf<USPCameraMode> BestCameraType = USPCameraMode::StaticClass();

	TSubclassOf<class USPCameraMode> CameraClass = GetCameraClassForCharacter(InViewTarget);
	if (CameraClass)
	{
...
void FSPCameraModeInstance::UpdateCamera(float DeltaTime, FTViewTarget& OutVT)
{
...
{
	bSkipBlendsOnNextUpdate = true;
	SkipNextInterpolation();
}

void ASPPlayerCameraManager::StopAmbientCameraShake(bool bImmediate)
{
	if (CameraBlendStack.IsValidIndex(0))
	{
		FActiveSPCamera& TopCamEntry = CameraBlendStack[0];
		if (TopCamEntry.Camera)
		{
			TopCamEntry.Camera->StopAmbientCameraShake(bImmediate);
		}
	}
}
...
void ASPPlayerCameraManager::StartAmbientCameraShake()
{
	if (CameraBlendStack.IsValidIndex(0))
	{
		...
		if (TopCamEntry.Camera)
		{
			TopCamEntry.Camera->StartAmbientCameraShake();
		}
	}
}