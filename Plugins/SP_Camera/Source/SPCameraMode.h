...
#include "SPCameraMode.generated.h"

class ASPPlayerCameraManager;
...
public:
	...
	virtual void OnBecomeActive(AActor* ViewTarget, USPCameraMode* PreviouslyActiveMode, bool bAlreadyInStack);

	/** Notify camera that it is no longer the primary camera.  Might still be processing as it blends out. */
	virtual void OnBecomeInactive(AActor* ViewTarget, USPCameraMode* NewActiveMode);

	/** Called by the SPCameraManager in UpdateViewTarget, camera mode implementations can use this to tailor their update logic as needed*/
	...
	UPROPERTY(transient)
	ASPPlayerCameraManager* PlayerCamera;
	...
	/** When true, camera will attempt to view through the ViewTarget's camera component */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "CameraSettings", meta = (EditCondition = "!bUseCineCamSettings"))
	bool bUseViewTargetCameraComponent;
	...
	/** Starts the specified ambient camera shake, if it's not already playing. */
	void StartAmbientCameraShake();

	/** Stops the specified ambient camera shake. Set immediate to true to ignore any blendouts and stop now */
...
protected:
	...
	/** Attempts to cast and return the camera modes's owning SP Player Controller */
