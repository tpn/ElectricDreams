...
#include "SPCam_ThirdPerson.generated.h"
...
protected:
	...
	/** Applied after PivotLocInterpolator, but only to the Z component of the pivot location. Useful for things like vehicles on bumpy ground, to have a loose Z but tighter XY */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "CameraSettings")
	FIIRInterpolatorFloat ExtraPivotLocZInterpolator = FIIRInterpolatorFloat(0.f);
	...
	/** This is the goal "neutral" pitch the lazy camera should drift toward. Ignored if bAllowLazyAutoFollowPitchControl is true. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AutoFollow")
	float LazyFollowGoalPitch = -10.f;
