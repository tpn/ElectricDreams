...
public:
	...
	const TSet<TObjectPtr<class AHoverDroneSpeedLimitBox>>& GetSpeedLimitBoxes() const
	{
		return SpeedLimitBoxes;
	}

	const TSet<TObjectPtr<class ABlockingVolume>>& GetBlockingVolumes() const
	{
...
private:
	...
	UPROPERTY(Transient)
	TSet<TObjectPtr<class AHoverDroneSpeedLimitBox>> SpeedLimitBoxes;

	UPROPERTY(Transient)
	TSet<TObjectPtr<class ABlockingVolume>> BlockingVolumes;
